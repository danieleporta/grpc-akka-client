...
import akka.stream.Materializer
  ...
  object GreeterServiceHandler {
     ...
     * several services.
     */
    def apply(implementation: GreeterService)(implicit mat: Materializer, system: ActorSystem): HttpRequest => scala.concurrent.Future[HttpResponse] =
     ...
     * several services.
     */
    def apply(implementation: GreeterService, eHandler: ActorSystem => PartialFunction[Throwable, io.grpc.Status])(implicit mat: Materializer, system: ActorSystem): HttpRequest => scala.concurrent.Future[HttpResponse] =
     ...
     */
    def apply(implementation: GreeterService, prefix: String)(implicit mat: Materializer, system: ActorSystem): HttpRequest => scala.concurrent.Future[HttpResponse] =
     ...
     * Registering a gRPC service under a custom prefix is not widely supported and strongly discouraged by the specification.
     */
    def apply(implementation: GreeterService, prefix: String, eHandler: ActorSystem => PartialFunction[Throwable, io.grpc.Status])(implicit mat: Materializer, system: ActorSystem): HttpRequest => scala.concurrent.Future[HttpResponse] =
     ...
     * Registering a gRPC service under a custom prefix is not widely supported and strongly discouraged by the specification.
     */
    def partial(implementation: GreeterService, prefix: String = GreeterService.name, eHandler: ActorSystem => PartialFunction[Throwable, io.grpc.Status] = GrpcExceptionHandler.defaultMapper)(implicit mat: Materializer, system: ActorSystem): PartialFunction[HttpRequest, scala.concurrent.Future[HttpResponse]] = {
      ...
      def handle(request: HttpRequest, method: String): scala.concurrent.Future[HttpResponse] = method match {
        ...
        case "ItKeepsReplying" =>
          ...
          GrpcMarshalling.unmarshal(request)(HelloRequestSerializer, mat)
            ...
            .map(e => GrpcMarshalling.marshalStream(e, eHandler)(HelloReplySerializer, mat, responseCodec, system))
